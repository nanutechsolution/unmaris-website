<x-layouts.app title="Beranda - Universitas Stella Maris Sumba">
    ...
    <section class="relative bg-unmaris-blue text-white overflow-hidden"
        ...
        <div class="relative min-h-[550px] md:min-h-[650px] lg:min-h-[750px] flex items-center">
            ...
            <div x-show="activeSlide === {{ $index }}"
                ...
                <div class="w-full lg:w-3/5 text-center lg:text-left lg:pr-10 pt-6 lg:pt-0">
                    ...
                    <h1 class="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold mb-4 md:mb-6 leading-[1.1] md:leading-tight drop-shadow-md">
                        {!! $slider->title !!}
                    </h1>

                    <p class="text-base sm:text-lg md:text-xl text-gray-200 mb-8 md:mb-10 max-w-2xl mx-auto lg:mx-0 leading-relaxed font-medium">
                        {{ $slider->description }}
                    </p>
                    ...
                    <div class="flex flex-col sm:flex-row justify-center lg:justify-start gap-3 md:gap-4">
                        ...
                        <a href="{{ $slider->button_url }}"
                            class="w-full sm:w-auto bg-unmaris-yellow text-unmaris-blue px-8 md:px-10 py-3.5 md:py-4 rounded-full font-black text-sm md:text-base uppercase tracking-widest hover:bg-yellow-400 hover:scale-105 transition-all shadow-[0_10px_30px_rgba(253,224,26,0.3)] text-center group">
                            {{ $slider->button_text ?? 'Lihat Selengkapnya' }}
                        </a>
                        @endif

                        <a href="{{ route('profile') }}" class="w-full sm:w-auto bg-white/10 backdrop-blur-sm border border-white/20 text-white px-8 md:px-10 py-3.5 md:py-4 rounded-full font-bold text-sm md:text-base hover:bg-white hover:text-unmaris-blue transition-all text-center">
                            Kenali UNMARIS
                        </a>
                    </div>
                </div>
            ...
            <div class="hidden sm:flex absolute inset-x-0 top-1/2 -translate-y-1/2 z-20 justify-between px-4 md:px-10 pointer-events-none">
                <button @click="activeSlide = activeSlide === 0 ? totalSlides - 1 : activeSlide - 1" class="pointer-events-auto w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white hover:bg-unmaris-yellow hover:text-unmaris-blue transition-all border border-white/20">
                    <svg class="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                    </svg>
                </button>
                <button @click="activeSlide = (activeSlide + 1) % totalSlides" class="pointer-events-auto w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white hover:bg-unmaris-yellow hover:text-unmaris-blue transition-all border border-white/20">
                    <svg class="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
